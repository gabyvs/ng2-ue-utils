import { StorageNonUnique } from './storage-non-unique';
...
class SomeTypeStorage extends StorageNonUnique<SomeType> {
...
class SomeOtherTypeStorage extends StorageNonUnique<SomeType> {
...
describe('Filters', () => {
    ...
    it('Create Filter with pattern & field', () => {
        const f = createFilter<{ a?; some?; b?; }>('some', 'b');
    ...
    it('Use filter with Product', () => {
        const fr = createFilter<SomeType>(new SomeType().name);
