...
const testWrappedError = {
    rejection: {
        originalError: new Error(testErrorMessage)
    }
};
...
const expectedEvent: IGAEventProps = {
    action: testErrorMessage,
    category: mockAppConfig.gtmErrorCategory,
    label: 'version: ' + mockAppConfig.appVersion,
    noninteraction: true,
    value: 0
};
const consoleErrorFunc = console.error; // Backup error function for restoration in afterEach()
...
function executeErrorHandler() {
    try {
        errorHandler.handleError(testWrappedError);
    } catch (e) {
        // Expected
    }
}
