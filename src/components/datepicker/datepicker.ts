...
export class DatePicker implements OnInit, OnChanges {
    ...
    private generateCalendar(year: number, month: number): string[][] {
        ...
        const generateDay = (day: number, _this): {} => {
            ...
            let styles: string[] = [];
            const max = (new Date(_this._beginDate + ((_this.maxRange - 1) * 3600 * 24 * 1000))).getTime();
            ...
            if (timeStamp === _this._beginDate) { styles.push('startdate'); }
            if (timeStamp === _this._endDate) { styles.push('enddate'); }

            // selected dates
            if (timeStamp >= _this._beginDate && timeStamp <= _this._endDate) {
                styles.push('between');

            // available tobe selected dates
            } else if (timeStamp > _this._beginDate && timeStamp <= max) {
